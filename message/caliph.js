...
caliph.sendMessage(m.chat, await tahta(args.join(' ')), 'imageMessage', { quoted: m, caption: 'Harta Tahta '+args.join(' ') })
break
case prefix+'stiker':
case prefix+'sticker':
case prefix+'s':
...
case prefix+'ttp':
  ...
  caliph.sendMessage(m.chat, res, messageType.sticker, { quoted: m })
  break
case prefix+'toimg':
case prefix+'stoimg':
...
if (!isAdmin) return m.reply('Perintah ini khusus admin grup!')
if (!args[0]) return m.reply('Teksnya?')
caliph.sendMessage(m.chat, args.join(' '), 'conversation', { contextInfo : { mentionedJid: groupMem.map(a => a.jid) }})
break
case prefix+'linkgc': 
